...
const testCases = [
  ...
  ['disco oats', 200, 350],
  ['red square energy drink', 100, 200],
  ['pizza mint condition', 250, 400],
  ['kombucha', 80, 180],
  ['fibratech bar', 120, 220],
...
async function test() {
  ...
  for (const [food, minCal, maxCal] of testCases) {
    try {
      ...
      const actualCal = parsed.calories || 0;
      const inRange = actualCal >= minCal && actualCal <= maxCal;
      console.log(`${food} -> ${actualCal} cal (expected ${minCal}-${maxCal}) ${inRange ? 'OK' : 'FAIL'}`);
      results.push({ food, expected: [minCal, maxCal], actual: actualCal, inRange, parsed });
      if (!inRange) failed.push({ food, minCal, maxCal, actual: actualCal, parsed });
    } catch(e) {
  ...
  console.log('\n---SUMMARY---');
  console.log(`Total: ${testCases.length}, Passed: ${testCases.length - failed.length}, Failed: ${failed.length}`);
  
  if (failed.length > 0) {
    console.log('\nFAILED ITEMS:');
    for (const f of failed) {
      console.log(`  ${f.food}: got ${f.actual}, expected ${f.minCal}-${f.maxCal}`);
    }
  }
  
  // Write failed items to file for overrides
  if (failed.length > 0) {
    fs.writeFileSync('/Users/brandonkatz/.openclaw/workspace/fitsorted/test-failed.json', JSON.stringify(failed, null, 2));
  }
}
