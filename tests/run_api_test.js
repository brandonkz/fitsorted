...
const newTestCases = [
  ['rooibos tea with 2 sugars', 40, 100],
  ['chicken licken sosati 4', 600, 1000],
  ['dischem whey protein scoop', 100, 180],
  ['spur coffee mate muffin', 250, 450],
  ['king pie cheese', 300, 500],
  ['nandos chicken wraps 2', 400, 700],
  ['pick n pay samoosa 3', 300, 500],
  ['ocean basket hake', 300, 500],
  ['woolworths avo half', 100, 200],
  ['spar biltong 50g', 100, 180]
];
...
async function runTests() {
  ...
  for (const [food, minCal, maxCal] of newTestCases) {
    ...
    const inRange = result.calories >= minCal && result.calories <= maxCal;
    console.log(`${inRange ? 'PASS' : 'FAIL'} | ${food} | Expected: ${minCal}-${maxCal} | Got: ${result.calories} | ${JSON.stringify(result)}`);
    results.push({ food, minCal, maxCal, actual: result.calories, result, inRange });
...
runTests().then(results => {
  ...
  const failed = results.filter(r => !r.inRange).length;
  console.log(`Total: ${results.length} | Passed: ${passed} | Failed: ${failed}`);
  failed > 0 && console.log('Failed items:', results.filter(r => !r.inRange).map(r => r.food).join(', '));
});