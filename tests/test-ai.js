...
const testCases = [
  ['dischem protein bar', 180, 250],
  ['tropika tropicol', 120, 200],
  ['rocomamas wings', 400, 650],
  ['galitos chicken wrap', 300, 500],
  ['ocean basket fish and chips', 550, 800],
  ['wow bowls salad', 250, 450],
  ['chicken licken lips', 450, 700],
  ['pick n pay pizza slice', 200, 350],
  ['russian with chips', 500, 750],
  ['stoney zero', 0, 20]
];
...
async function testItem(food, minCal, maxCal) {
  try {
    ...
    console.log(JSON.stringify({ food, expected: minCal + '-' + maxCal, got: cal, inRange, result: JSON.stringify(result) }));
    return { food, minCal, maxCal, got: cal, inRange, result };
  } catch (e) {
    console.log(JSON.stringify({ food, error: e.message }));
    return { food, minCal, maxCal, error: e.message };
  }
}
...
(async () => {
  ...
  for (const [food, minCal, maxCal] of testCases) {
    const r = await testItem(food, minCal, maxCal);
    results.push(r);
  }
  console.log('\n--- SUMMARY ---');
  console.log('Total tested:', results.length);
  console.log('Passed:', results.filter(r => r.inRange).length);
  console.log('Failed:', results.filter(r => !r.inRange && !r.error).length);
  console.log('Errors:', results.filter(r => r.error).length);
  console.log('\nFailures:');
  results.filter(r => !r.inRange && !r.error).forEach(r => {
    console.log(' - ' + r.food + ': got ' + r.got + ', expected ' + r.minCal + '-' + r.maxCal);
  });
})();